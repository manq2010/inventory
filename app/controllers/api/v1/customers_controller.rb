class Api::V1::CustomersController < ApplicationController
    ...
    def create
      ...
      if customer.save
        render json: { operation: "Customer created successfully with id: #{customer.id}", data: { customer_id: customer.id } }, status: :created
      else
    ...
    def update
      ...
      if customer.update(customer_params)
        render json: { operation: "Customer updated successfully with id: #{customer.id}", data: { customer_id: customer.id } }, status: :ok
      else
        render json: { operation: 'Not successful', data: { errors: customer.errors } }, status: :bad_request
      end
    end
    ...
    def destroy
      ...
      if customer&.destroy
        ...
        render json: { operation: "Couldn't delete customer with id #{params[:id]}.", data: { errors: { customer: 'Not Found' } } }, status: :not_found
      end
    end
